...
const randomPassword = Array.from(Array(8), () =>
...
const CreateClient = ({route, navigation}) => {
  ...
  const createUser = async () => {
    ...
    try {
      ...
      const newUser = {
        ...user,
        team: {name: clientTeam.name, id: clientTeam.id},
        email: user.email.toLocaleLowerCase(),
        password: randomPassword,
        upliner: {name: `${client.name} ${client.lastName}`, id: client.id},
      };
      const createdClientsRef = firebase
        .firestore()
